	...
	$(function(){
		...
		$('.container form').one('submit', function (e) {
			$('.container form').prev().html("수정 페이지");
	      	...
	      	$('.container form button').eq(2).one('click',function(){
	      		$('.container form').prev().html("상세 페이지");
		      	$('.container form input').filter(":gt(0)").removeProp('readonly', true);
		      	$('.container form button').eq(1).text("삭제").attr('type','button').addClass('btn-danger')
	      	});
	      	return false;
		});
		
		$('.container form button').eq(3).click(function(e){
			history.back();
		});
			...
			<div class="row">
				...
				<form class="form-control" method="post">
					<div class="form-group">
						...
						<input type="number" name="empno" id="empno" value="<%=bean.getEmpno() %>" placeholder="사번" readonly>
					</div>
					<div class="form-group">
						<label for="ename">ename</label>
						<input type="text" name="ename" id="ename" value="<%=bean.getEname() %>" placeholder="이름" readonly>
					</div>
					<div class="form-group">
						<label for="sal">sal</label>
						<input type="number" name="sal" id="sal" value="<%=bean.getSal() %>" placeholder="연봉" readonly>
					</div>
					<div class="form-group">
